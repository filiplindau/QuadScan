...
class ProcessPoolTask(Task):
    ...
    def result_thread_func(self):
        ...
        while self.stop_result_thread_flag is False:
            ...
            try:
                result = self.out_queue.get(True, 0.05)
            except multiprocessing.queues.Empty:
                continue
...
class ProcessPoolTaskPool(Task):
    """
    Start up a process pool that can consume data. Emit trigger when queue is empty.
    ...
    def pool_callback(self, result):
        self.logger.debug("Pool callback returned {0}".format(result))
        self.completed_work_items += 1
        self.result = result
...
def process_image_func2(image, k_ind, k_value, image_ind, threshold, roi_cent, roi_dim, cal=[1.0, 1.0], kernel=3,
                       bpp=16, normalize=False, enabled=True):
    # print("Processing image {0}, {1} in pool, size {2}".format(k_ind, image_ind, image.shape))
    t0 = time.time()
    try:
...
if __name__ == "__main__":
    ...
    im_list = [np.random.random((1280, 1024)) for k in range(10)]
    ...
    # Process image func
    # t = ProcessPoolTask(process_image_func, name="test", callback_list=[callback_f])
    t = ProcessPoolTaskPool(process_image_func2, name="test", callback_list=[callback_f])
    t.start()
    time.sleep(1.0)
...
#                     roi_dim=roi_d, cal=cal, kernel=kern, bpp=16, normalize=norm, enabled=en)
    ...
    for im in im_list:
        logger.info("Adding item {0}".format(k_ind))
        t.add_work_item(im, k_ind, k_val, im_ind, th, roi_c, roi_d, cal, kern, bpp=16, normalize=norm, enabled=en)
        k_ind += 1
