...
class QuadScanGui(QtWidgets.QWidget):
    ...
    def setup_layout(self):
        ...
        self.ui.fit_widget.points_clicked_signal.connect(self.points_clicked2)
        self.ui.charge_widget.points_clicked_signal.connect(self.points_clicked2)
    ...
    def prepare_plots(self, scan_type="single"):
        ...
        with self.gui_lock:
            ...
            col_fit_y = (91, 22, 76)
            if scan_type == "single":
                self.ui.charge_widget.add_curve("charge", symbol="o", color=col_charge,
                ...
                self.ui.charge_widget.set_y_link("fit_x", "sigma_x")
                self.ui.charge_widget.set_legend_position("right")
                ...
                self.ui.fit_widget.add_curve("fit_el", pen=pq.mkPen(col_fit_x, width=2), width=2, visible=True)
                self.ui.fit_widget.set_y_link("ab_x", "fit_el")
    ...
    def plot_sigma_data(self):
        ...
        self.ui.charge_widget.set_data(x_data=k, y_data=sigma_x, curve_name="sigma_x",
                                       symbol=sigma_symbol_list, auto_range=True)
        self.ui.charge_widget.set_data(x_data=k, y_data=sigma_y, curve_name="sigma_y",
                                       symbol=sigma_symbol_list, auto_range=True)
        self.ui.charge_widget.set_data(x_data=k, y_data=q, curve_name="charge",
                                       symbol=sigma_symbol_list, auto_range=True)
        ...
        self.ui.fit_widget.set_data(x_data=np.array(a), y_data=np.array(b), curve_name="ab_x",
                                    symbol=sigma_symbol_list, width=None)
        mq = MultiQuadLookup()
        psi = np.linspace(0, 2 * np.pi, 500)
        th, r_maj, r_min = mq.calc_ellipse(self.fit_result.alpha, self.fit_result.beta,
                                           self.fit_result.eps, np.maximum(np.min(sigma), 0.1e-3))
        ae, be = mq.get_ab(psi, th, r_maj, r_min)
        self.ui.fit_widget.set_data(x_data=ae, y_data=be, curve_name="fit_el")
...
#                                    measure_interval=1.0,
    ...
    def multiquad_scan_callback(self, task):
        ...
        if not task.is_done():
            ...
            if not task.is_cancelled():
                ...
                self.update_image_selection(step["image_p"].pic_roi, auto_levels=True, auto_range=True)
            ...
            self.quad_scan_data_analysis = self.quad_scan_data_scan
            root.info("Acc parameters: {0}".format(self.quad_scan_data_scan.acc_params))
            self.ui.p_image_index_slider.setMaximum(len(self.quad_scan_data_analysis.images) - 1)
            self.ui.p_image_index_slider.setValue(0)
            self.ui.p_image_index_slider.update()
            root.debug("Proc images len: {0}".format(len(self.quad_scan_data_scan.proc_images)))
            root.debug("Images len: {0}".format(len(self.quad_scan_data_scan.images)))
            self.user_enable_list = [True for x in range(len(self.quad_scan_data_scan.proc_images))]
            self.update_analysis_parameters()
            # self.update_proc_image_signal.emit(None)
            self.update_image_selection()
    ...
    def points_clicked2(self, plot_name, point_list):
        root.info("Plot {0} clicked. Points {1}. Distance {2}".format(plot_name, [p.index() for p in point_list], [p.dist for p in point_list]))
        en_data = self.user_enable_list
        ind = point_list[0].index()
        dist = point_list[0].dist
        for p in point_list:
            if p.dist < dist:
                ind = p.index()
                dist = p.dist
